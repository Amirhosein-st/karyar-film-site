...
const GenreFilmListPage = () => {
  ...
  const fetchMovies = async (page) => {
    ...
    try {
      const genre = localStorageService.getItem('NumberGenre');
      ...
      localStorageService.setItem('currentPage', response.data.metadata.current_page);
    } catch (error) {
      console.log(error);
    }
  ...
  const namegenre = localStorageService.getItem('NameGenre');
          ...
          {loading ? (
            ...
            movies.map((movie) => (
              <div className="film-detailss" key={movie.id} onClick={() => handleMovieClick(movie.id)} >
                ...
                <button id="bookmark-filmlist" className={ bookmarks.some((b) => b.id === movie.id) ? 'bookmark-yellow' : 'bookmark'} onClick={(e) => {e.stopPropagation();
                    handleBookmarkClick(movie.id, movie.title)}}> Bookmark </button>
              </div>
            ))
          )}
        </div>
